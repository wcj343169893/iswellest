...
DateSelector.prototype = {
  initialize: function(oYear, oMonth, oDay, options) {
    ...
    //年设置
    this.SetSelect(this.SelYear, this.MinYear, this.MaxYear - this.MinYear + 1, this.SelYear.value);
    //月设置
    this.SetSelect(this.SelMonth, 1, 12, this.SelMonth.value);
    //日设置
    this.SetDay();
  ...
  SetDay: function() {
    ...
    this.SetSelect(this.SelDay, 1, daysInMonth, this.SelDay.value);
  },
  //select设置
  SetSelect: function(oSelect, iStart, iLength, selected) {
    //添加option
    iIndex = 0;
    oSelect.options.length = iLength;
    for (var i = 0; i < iLength; i++) { 
        oSelect.options[i].text = oSelect.options[i].value = iStart + i;
        if (1*selected == oSelect.options[i].value) {
            iIndex = i;
        }
    }
    //设置选中项
