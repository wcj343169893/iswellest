...
class ProductController extends AppController {
    var $name       = 'Product';
    var $uses       = array('Product', 'Category', 'Brand', 'ProductPhoto', 'ProductDesc', 'StockInfo');
    var $components = array('Query', 'PageSetting','ConfigIni');
    ...
    function index() {
        ...
        if (!empty($this->params['named']['brand_id'])) {
            ...
            //ブランド情報
            $brandList = $this->Brand->getList(array($this->params['named']['brand_id']), 1);
    ...
    function detail() {
        ...
        if (!empty($productInfo['Product']['bonus_product_cds'])) {
            $productInfos = array($productInfo['Product']['product_cd'] => $productInfo);
        ...
        if (empty($browsedProductCds) 
                || (!empty($browsedProductCds) && !in_array($productCd, $browsedProductCds))) {
    ...
    function cached_relation_product() {
        ...
        if (empty($this->params['named']['product_cd']) 
                || empty($this->params['named']['relation_product_cds'])) {
            return array();
        }
    ...
    function __getDeliveryDay(&$productInfo, $subScriptStatus) {
        ...
        } else if ($productInfo['Product']['product_type'] == PRODUCT_TYPE_RESERVATION && in_array($productInfo['Product']['sales_status'], array(PRODUCT_SALES_STATUS_NORMAL, PRODUCT_SALES_STATUS_STOCK_ONLY, PRODUCT_SALES_STATUS_WILL_END_SELLING))) {
