...
<div id="categoryProduct<?php echo $key;?>">
    ...
    <div class="column clearfix">
        ...
        <p>
            标&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签&nbsp;&nbsp; 
            <?php echo $appForm->text("{$pageSettingType}.category_product.{$key}.label", array('class'=>'input_400'));?>
            <?php echo $appForm->error("{$pageSettingType}.category_product.{$key}.label", '标签');?>
            <span class="gray">标签之间用","或空格分隔</span>
        </p>
        <p class="line">
            ICON路径 
            <?php echo $appForm->text("{$pageSettingType}.category_product.{$key}.icon_url", array('class'=>'input_400'));?>
            <?php echo $appForm->error("{$pageSettingType}.category_product.{$key}.icon_url", 'ICON路径');?>
        </p>
        <div class="clearfix">
            ...
            <?php for($i=1;$i<=3;$i++):?>
                <div class="ad">
                    ...
                    <p>
                        ...
                        <?php echo $appForm->file("{$pageSettingType}.category_product.{$key}.right_ad.{$i}.pic", array('name'=>'filename','id'=>"rightAd{$i}{$key}Pic", 'size'=>'8', 'onchange'=>"ajaxUploadImg(this, '".UPLOAD_PIC_ROOT.DS.splitWordsWithUnderLine($pageSettingType)."/{$id}/category_product/{$key}/right_ad/{$index}/', '240', '125', 'pRightAd{$i}{$key}Pic', 'rightAd{$i}{$key}PicPath');"));?>
                        <?php echo $appForm->error("{$pageSettingType}.category_product.{$key}.right_ad.{$i}.path", '图片');?>
                    </p>
                    <p class="f_11 gray">jpg,png,gif,尺寸240×125,大小400K内</p>
            ...
            <div class="leftContent">
                <div class="leftAd">
                    ...
                    <p>
                        ...
                        <?php echo $appForm->file("{$pageSettingType}.category_product.{$key}.leftmain_ad.pic", array('name'=>'filename','id'=>"leftmainAd{$key}Pic", 'size'=>'8', 'onchange'=>"ajaxUploadImg(this, '".UPLOAD_PIC_ROOT.DS.splitWordsWithUnderLine($pageSettingType)."/{$id}/category_product/{$key}/left_ad/{$index}/', '200', '380', 'pLeftmainAd{$key}Pic', 'leftmainAd{$key}PicPath');"));?>
                        <?php echo $appForm->error("{$pageSettingType}.category_product.{$key}.leftmain_ad.path", '图片');?>
                    </p>
                    <p class="f_11 gray">jpg,png,gif,尺寸200×380,大小400K内</p>
