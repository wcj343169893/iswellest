...
<div class="ad" id="focusPic<?php echo $key;?>">
    ...
    <p>
        ...
        <?php echo $appForm->file("{$pageSettingType}.focus_pic.{$key}.file", array('name'=>'filename', 'size'=>'8', 'onchange'=>"ajaxUploadImg(this, '".UPLOAD_PIC_ROOT.DS.splitWordsWithUnderLine($pageSettingType)."/focus/{$id}/', '790', '200', 'pFocusPic{$key}', 'focusPicPath{$key}');"));?>
        <?php echo $appForm->error("{$pageSettingType}.focus_pic.{$key}.path", '图片');?>
    </p>
    <p class="f_11 gray">jpg,png,gif,尺寸790×200,大小400K内</p>
