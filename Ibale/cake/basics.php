 ...
 */
	 ...
	 * @return 缓存内容
	 * */
	function readCache($path,$expires = '+1 day'){
		if(empty($path)){
			return false;
		}
		$path=strtolower($path);
		$data =  cache($path, null, $expires, 'custom');
		if(empty($data)){
			return false;
		}
		$read_data=unserialize($data);
		return $read_data;
	}
	/**
	 * @todo 写入缓存
	 * @return 是否缓存成功
	 * */
	function writeCache($data = null,$path="",$expires = '+1 day'){
		if(empty($path)){
			return false;
		}
		$path=strtolower($path);
		...
		return cache($path, $write_data, $expires, 'custom');
	}
 ...
 */
//modify by shilei, by 2012-01-05
/**
