...
class ProductsController extends AppController {
	...
	public function index() {
		...
		if (! empty ( $_REQUEST ["cid"] )) {
			$paginate ["conditions"] = array (
					"Product.cateID" => intval ( $_REQUEST ["cid"] ) 
			);
		}
		$searchkey="";
		if (! empty ( $_REQUEST ["q"] )) {
			$searchkey = h ( $_REQUEST ["q"] );
			$key = "'%$searchkey%'";
			$paginate ["conditions"] = array (
					"Product.name like " . $key 
			);
		}
		
		$this->paginate = $paginate;
		$this->Product->recursive = 0;
		$this->set ( 'key', $searchkey );
	...
	public function search($name = null) {
		...
		// old search code
		/*
		 * $conditions = array('name LIKE' => $name); $this->set('products',
		 * $this->Product->find('all', array( 'condition' =>
		...
		if (! isset ( $this->data ['Search'] )) {
			...
			// this will be executed if the user enters a name and selects a
			// category
			if ($this->data ['Product'] ['pname'] != '' && $this->data ['Product'] ['cat'] != '') {
				$conditions = array (
						"Product.cateID" => $this->data ['Product'] ['cat'],
						"Product.name LIKE" => "%" . $this->data ['Product'] ['pname'] . "%" 
				);
				$this->paginate = array (
