...
// 定义函数：用于联动省 和 市 两级-----city为市级下拉框的id名，pro为省下拉框的id 号。
...
function changepro(city, pro) {
	...
	for (i = 0; i < lmcount; i++) {
		if (lm[i][1] == pro) {
			document.getElementById(city).options[document.getElementById(city).length] = new Option(
					lm[i][0], lm[i][0]);
		}
	}

}
// 定义函数：用于联动市 和 县 两级-----county 为县级下拉框的id名，city为市下拉框的id 号。
...
function changecity(county, city) {
	...
	for (j = 0; j < lmcount2; j++) {
		if (lm2[j][1] == city) {
			document.getElementById(county).options[document
					.getElementById(county).length] = new Option(lm2[j][0],
					lm2[j][0]);
		}
	}
