...
class Index {
	function index() {
		...
		//即将下架商品
		$endingsoondata=$goods->limit(12)->order ( "sale_end_time asc" )->where(array ('sale_end_time > ' =>$d ))->joins ( "category", "cat_id", "cat_id", "cat_id,cat_name" )->select();
		$this->assign ( 'endingsoondata', $endingsoondata );
		//即将上架（明天）
		$td_time=date ( "Ymd", strtotime ( "+1 day" ));
		$tomorrowData=$goods->limit(10)->where(array("DATE_FORMAT(FROM_UNIXTIME(sale_begin_time),'%Y%m%d')"=>$td_time))->joins ( "category", "cat_id", "cat_id", "cat_id,cat_name" )->select();
// 		print_r($tomorrowData);
		$this->assign ( 'tr1', date ( "l m/d", strtotime ( "+1 day" ) ) );
		$this->assign ( 'tr1Data', $tomorrowData );
		//即将上架（后天）
		$td_time=date ( "Ymd", strtotime ( "+2 day" ));
		$tomorrow2Data=$goods->limit(10)->where(array("DATE_FORMAT(FROM_UNIXTIME(sale_begin_time),'%Y%m%d')"=>$td_time))->joins ( "category", "cat_id", "cat_id", "cat_id,cat_name" )->select();
		$this->assign ( 'tr2', date ( "l m/d", strtotime ( "+2 day" ) ) );
		$this->assign ( 'tr2Data', $tomorrow2Data );
		//即将上架（大后天）
		$td_time=date ( "Ymd", strtotime ( "+3 day" ));
		$tomorrow3Data=$goods->limit(10)->where(array("DATE_FORMAT(FROM_UNIXTIME(sale_begin_time),'%Y%m%d')"=>$td_time))->joins ( "category", "cat_id", "cat_id", "cat_id,cat_name" )->select();
		$this->assign ( 'tr3', date ( "l m/d", strtotime ( "+3 day" ) ) );
		$this->assign ( 'tr3Data', $tomorrow3Data );
		//创意专区
		$creativeData=$goods->limit(6)->where(array("is_best"=>1))->joins ( "category", "cat_id", "cat_id", "cat_id,cat_name" )->select();
		$this->assign ( 'datar', $creativeData );
...
// 		$l = D ( 'link' );
// 		// 特价商品
// 		$data = $s->limit ( 6 )->where ( array (
// 				'w_type' => 'tj' 
// 		) )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		$this->assign ( 'data', $data );
// 		// 创意专区
// 		$data_r = $s->limit ( 3 )->where ( array (
// 				'w_type' => 'r' 
// 		) )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
...
// 		) )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		// $result=$s->unit_select("SELECT w.*,c.c_name FROM pro_ware w LEFT
// 		// JOIN pro_cat c ON w.w_cat=c.id WHERE sale_begin_time <> '' AND
// 		// DATE_FORMAT(sale_begin_time,'%Y%m%d%h%i')>DATE_FORMAT(NOW(),'%Y%m%d%h%i')
// 		// ORDER BY sale_begin_time ASC LIMIT 0,10;");
		
// 		$result1 = $s->limit ( 6 )->order ( "sale_begin_time asc" )->where ( array (
// 				'w_type' => 'tj' 
// 		) )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		// print_r($s->joins("cat","w_cat","id","id,c_name")->select());
// 		$this->assign ( 'newData', $newData );
// 		// 快要过期产品
// 		$endingsoondata = $s->limit ( 12 )->order ( "sale_end_time asc" )->where ( array (
// 				'sale_end_time > ' => date ( "Y-m-d H:i:s" ) 
// 		) )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		$this->assign ( 'endingsoondata', $endingsoondata );
// 		// 明天上线的产品
// 		// echo "明天:".date("Y-m-d",strtotime("+1 day")). "<br>";
// 		$tr1Data = $s->limit ( 10 )->where ( "sale_begin_time like '" . date ( "Y-m-d", strtotime ( "+1 day" ) ) . "%'" )->order ( "sale_begin_time asc" )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		$this->assign ( 'tr1', date ( "l m/d", strtotime ( "+1 day" ) ) );
// 		$this->assign ( 'tr1Data', $tr1Data );
// 		// print_r($tomorrowData);
// 		// 后天上线的产品
// 		$tr2Data = $s->limit ( 10 )->where ( "sale_begin_time like '" . date ( "Y-m-d", strtotime ( "+2 day" ) ) . "%'" )->order ( "sale_begin_time asc" )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		$this->assign ( 'tr2', date ( "l m/d", strtotime ( "+2 day" ) ) );
// 		$this->assign ( 'tr2Data', $tr2Data );
// 		// 大后天上线的产品
// 		$tr3Data = $s->limit ( 10 )->where ( "sale_begin_time like '" . date ( "Y-m-d", strtotime ( "+3 day" ) ) . "%'" )->order ( "sale_begin_time asc" )->joins ( "cat", "w_cat", "id", "id,c_name" )->select ();
// 		$this->assign ( 'tr3', date ( "l m/d", strtotime ( "+3 day" ) ) );
...
// 		$this->assign ( 'goodsList', $goodsList );
		...
		// $c=D('cat');
		// //高老师写的联合查询便利数组，没太明白。。。
		// $cat=$c->where(array("pid"=>0))->r_select(array("cat", '', "pid",
		// array("subcat", 'id desc', 15)));
		// $this->assign('cat',$cat);
		// print_r($cat);
		// 右侧公告
